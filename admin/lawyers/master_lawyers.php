...
<form method="GET">
    ...
    <a href="add_master_lawyer.php" class="btn" style="background:#52b788;">➕ إضافة محامي جديد</a>
</form>
...
<tr>
    ...
    <td><?= htmlspecialchars($l['master_id']) ?></td>
    <td><?= htmlspecialchars($l['office_address'] ?? '-') ?></td>
    <td><?= htmlspecialchars($l['created_at'] ?? '-') ?></td>
    <td>
        <a href="edit_master_lawyer.php?id=<?= $l['master_id'] ?>" class="btn edit-btn">تعديل</a>
        <a href="delete_master_lawyer.php?id=<?= $l['master_id'] ?>" class="btn delete-btn" onclick="return confirm('هل أنت متأكد من حذف هذا المحامي؟')">حذف</a>
    </td>
</tr>
<?php endforeach; ?>
