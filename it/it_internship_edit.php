...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) {
  echo "<p class='error'>❌ معرّف الفرصة غير صحيح.</p>";
  include __DIR__ . "/includes/it_provider_layout_footer.php";
  exit;
}

// جلب الفرصة والتأكد أنها للمزود الحالي
$stmt = $pdo->prepare("
  SELECT *
  FROM it_internships
...
if (!$internship) {
  echo "<p class='error'>❌ لا تملك صلاحية تعديل هذه الفرصة أو أنها غير موجودة.</p>";
  include __DIR__ . "/includes/it_provider_layout_footer.php";
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
  try {
    // حذف الطلبات أولاً (إذا عندك FK قد تحتاج هذا)
  ...
  <div class="auth-grid">
    ...
    <div class="auth-field col-6">
      ...
      <div class="muted" style="margin-top:6px;">
        نشرها لأول مرة سيحدد تاريخ النشر تلقائياً.
      </div>
    </div>
